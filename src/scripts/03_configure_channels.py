...
Applies the setup the calibration expects on the CTC100 we own directly:
  ...
  * heater = 100 W output (Out1) -> units = Amps, high-limit safety clamp
  * vsense channel (AIO2)        -> 'Input'

The 100 W outputs are dedicated outputs (no IOtype to set). The 40 K sensor lives
on a different CTC (owned by FridgeControl) and is read over RPC -- not here.

Optional: on the CTC100, Monitors -> Show enables the output card's measured
heater voltage/current/resistance channels (useful for cross-checks).
...
def main() -> None:
    ...
    try:
        ...
        ctc.set_units(ch.heater, cfg.heater_units)          # Amps
        ...
        print(f"  {ch.heater}: 100 W output, units {cfg.heater_units}, HiLmt {cfg.heater_hilmt}")
        print(f"  {ch.vsense}: Input (4-wire heater voltage sense)")
        print("\nReadback:")
        print(f"  A {ch.sensor_a} = {ctc.read_channel(ch.sensor_a):8.3f} K")
        print(f"  heater {ch.heater} = {ctc.read_channel(ch.heater):8.4f} A (should be ~0)")
